...
#'
#' @param object `[glm, fit_logit, fit_clogit, glmmTMB]` \cr A fitted RSF or (i)SSF model.
...
prep_uhc <- function(object, test_dat,
  ...
  # Check inputs
  ## object
  if(!inherits(object, c("glm", "fit_logit", "fit_clogit", "glmmTMB"))){
    stop(paste0("'object' should be an object of class 'glm', 'fit_logit', ",
                "'fit_clogit',", " or 'glmmTMB'."))
  }
...
#' @rdname prep_uhc
#' @export
prep_uhc.glmmTMB <- function(object, test_dat,
                             n_samp = 1000, n_dens = 512,
                             verbose = TRUE) {
  ...
  b <- summary(object)$coef$cond[, 1]
  S <- stats::vcov(object)[[1]]
  gc()
  object$frame <- object$frame[0, ]
  pred <- predict(object, newdata = test_dat, se.fit = TRUE, re.form = NULL, allow.new.levels = T)
...
prep_test_dat.glmmTMB <- function(object, test_dat, verbose = TRUE) {
  ...
  if (verbose) {
    message("Assuming stratum variable contains the string 'step_id' ")
  }
  strat <- preds[grep("step_id", preds, fixed = TRUE)]
  ...
  del <- c("x1_", "x2_", "y1_", "y2_", "t1_", "t2_", "dt_", "sl_", "ta_", "step_id_",
...
#' @param newdata `[data.frame]` \cr A `data.frame` to predict HSF values.
#'
...
#'
#' @param object `[glmmTMB]` Fitted iSSF.
#' @param l `[list]` List returned by `prep_test_dat.glmmTMB()`
#'
issf_drop_stratum <- function(object, l) {
  # Get formula terms from object
  tt <- terms(stats::formula(object))
  
  # Get location of stratum
  ss <- grep("step_id", attr(tt, "term.labels"), fixed = TRUE)
...
#' Create formula without random effects/slopes from iSSF
#'
...
issf_drop_randoms <- function(object, l) {
  ...
  # check if intercept is in model
  if ("(Intercept)" %in% tt) {
    tt <- tt[which(tt) != "(Intercept)"]
  } else{
    tt <- c(-1, tt)
  }
  ...
  form <- paste0( resp, '~', paste(tt, collapse =' + '))
