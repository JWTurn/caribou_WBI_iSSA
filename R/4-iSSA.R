...
tar_load(stepID)
saveRDS(stepID, file.path(derived, 'sk_derived.RDS'))

dat <- stepID
dat[,id:=as.factor(id)]
dat[,lc_end_adj := lc_end]
dat[lc_end %in% c('barren', 'grassland', 'lichen-grass', 'shrub', 'urban'),lc_end_adj:= 'open-forage']
...
summary(mod)
saveRDS(mod, file.path(derived, 'sk_ssa.RDS'))
